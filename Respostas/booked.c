...
{
    Leitor *leitor = (Leitor *)buscaDadoNaLista(alexandria->leitores, id1, getIdLeitor);
...
void recomendarLivroED(bookED *alexandria, int id1, int id2, int id3)
{
    ...
    if (!leitor2)
    {
        fprintf(alexandria->fSaida, "Erro: Leitor destinatário com ID %d não encontrado\n", id3);
    }
    else if (!leitor1)
    {
        fprintf(alexandria->fSaida, "Erro: Leitor recomendador com ID %d não encontrado\n", id1);
    }
    else if (!livro)
    {
        fprintf(alexandria->fSaida, "Erro: Livro com ID %d não encontrado\n", id2);
    ...
    {
        fprintf(alexandria->fSaida, "%s não precisa da recomendação de \"%s\" pois já leu este livro\n", getNomeLeitor(leitor2), getTituloLivro(livro));
    }
    else if (livro == getLivroDesejadoId(leitor2, id2))
    {
        fprintf(alexandria->fSaida, "%s já deseja ler \"%s\", recomendação desnecessária\n", getNomeLeitor(leitor2), getTituloLivro(livro));
    }
    else
    {
...
void processarRecomendacaoED(bookED *alexandria, int id3, int id2, int id1, int yesno)
{
    ...
    else
    {
        ...
        if (!recomendacao)
        {
            fprintf(alexandria->fSaida, "%s não possui recomendação do livro ID %d feita por %s\n", getNomeLeitor(leitor2), id2, getNomeLeitor(leitor1));
        }
        else
        {
